<?php
	...
	{	
		...
		$query="INSERT INTO order ( status,total_price,quantity,customer_id) VALUES ('\"pending\"', {$_SESSION['totalprice']},{$count},{$_SESSION['login']});";
		...
		else
		{
			...
			for($i=0;$i<$count;$i++)
				{
					$query="INSERT INTO ordered_product (order_id,product_id) VALUES ('{$last_id}','{$_SESSION['shopping_cart'][$i]}');";
					$result=mysqli_query($connected,$query);
					if(!$result)
					{
					echo mysqli_error($connected);
